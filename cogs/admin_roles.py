...
class AdminSettings(commands.Cog):
    ...
    async def removeadmin(self, ctx: commands.Context, role: discord.Role):
        data = settings.load_settings()
        admin_roles_ids = set(data.get("admin_roles", []))
    ...
    @commands.hybrid_command(
        name="listadmins",
        with_app_command=True,
        description="Показывает список административных ролей"
    )
    async def listadmins(self, ctx: commands.Context):
        ...
        for rid in admin_roles_ids:
            ...
            embed.add_field(
                name=role.name if role else "❓ Неизвестная роль",
